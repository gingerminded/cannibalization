...
proc sql;
  ...
  select t1.product,ksubstr(t1.product_nm,1,min(klength(t1.product_nm),40)) as product_nm length=160, /*текст обрезается до 40 символов из-за ограничений Forecast Studio*/
      ksubstr(t1.ui1_nm,1,min(klength(t1.ui1_nm),40)) as ui1_nm length=160,
      t1.ui2,ksubstr(t1.ui2_nm,1,min(klength(t1.ui2_nm),40)) as ui2_nm length=160,
      t1.ui3,ksubstr(t1.ui3_nm,1,min(klength(t1.ui3_nm),40)) as ui3_nm length=160,
  ...
  from items_c t1 
  where t1.ui1_nm in ('Яйцо' /* 'Охлажденное мясо'*/)
...
quit;
proc sql;
create table nms as select distinct product_nm, min(klength(product_nm),40) as len
from items_c
;
quit;
